...
func generateDetails(width: int, height: int):
	...
	var multimesh = MultiMesh.new()
	multimesh.transform_format = MultiMesh.TRANSFORM_3D
	multimesh.instance_count = width * height / 4
	multimesh.visible_instance_count = multimesh.instance_count
	...
	for i in multimesh.visible_instance_count:
		...
		var scaleH = random.randf_range(0.5, 1.5)
		var h = image2.get_pixel(int(x) % image.get_width(), int(y) % image.get_height()).r
		
		multimesh.set_instance_transform(i, Transform3D(Basis(), Vector3(0.0, 0.0, 0.0)).rotated(Vector3(0.0, 1.0, 0.0), PI/4).scaled(Vector3(scaleH, random.randf_range(0.5, 2.0), scaleH)).translated(Vector3(x, 1.0 - h * 4.0, y)))
