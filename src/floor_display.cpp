...
void FloorDisplay::_draw() {
    ...
    double rootX = cameraPivot->get_position().x - 50;
    double rootY = cameraPivot->get_position().z - 130 + 100;
    ...
    for(int i = -1; i <= 1; i++) {
        for(int j = -1; j <= 1; j++) {
            double x = rootX + i * current->getSettings()->width;
            double y = rootY + j * current->getSettings()->height;
