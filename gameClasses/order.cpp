...
*/
void Order::updateOrders(const std::multiset<std::shared_ptr<Order>, OrderOrder> &orders) const {
    for(auto &order : orders) {
        if(getID() > order->getReferenceID() && order->getSenderID() != getSenderID()) order->adjustIDs(GameObject::getIDCounter());
    }
}

bool OrderOrder::operator()(const std::shared_ptr<Order> &lhs, const std::shared_ptr<Order> &rhs) const 
{
